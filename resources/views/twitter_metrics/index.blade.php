<x-layouts.app :title="__('Twitter Influencers')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="grid auto-rows-min gap-4 md:grid-cols-3">
            <div class="flex items-center justify-center aspect-video rounded-xl border border-blue-300 bg-white shadow-lg dark:bg-neutral-900 dark:border-neutral-700 hover:shadow-xl transition-shadow duration-300">
                <div class="text-center space-y-2">
                    ...
                    <p class="text-5xl font-extrabold text-blue-600 dark:text-blue-400">
                        {{ $influencers->whereHas('socialProfiles', fn($q) => $q->whereHas('platform', fn($q) => $q->where('name', 'Twitter')))->count() }}
                    </p>
        ...
        <div class="relative h-full flex-1 overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
            <h2 class="text-xl font-semibold text-neutral-800 dark:text-white mb-4">Twitter Influencers</h2>
                ...
                <table class="w-full text-sm text-left border-separate border-spacing-y-2">
                    <thead>
                        <tr class="text-neutral-600 dark:text-neutral-300">
                            ...
                            <th class="py-3 px-4">Usuario Twitter</th>
                            <th class="py-3 px-4">Seguidores</th>
                            <th class="py-3 px-4">Tweets</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($influencers->filter(fn($i) => $i->hasTwitterProfile()) as $influencer)
                            @php
                                $twitterProfile = $influencer->socialProfiles->firstWhere('platform.name', 'Twitter')
                            @endphp
                            <tr class="bg-white dark:bg-neutral-800 shadow-sm rounded-md">
                                <td class="py-2 px-4">
                                    <div class="w-[50px] h-[50px]">
                                        <a href="{{ route('twitter.show', $twitterProfile->id) }}"
                                ...
                                <td class="py-2 px-4">
                                    <a href="{{ $twitterProfile->profile_url }}" target="_blank"
                                ...
                                <td class="py-2 px-4 text-neutral-700 dark:text-neutral-300">
                                    {{ $twitterProfile->twitterMetrics->sum('tweets') ?? 0 }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
